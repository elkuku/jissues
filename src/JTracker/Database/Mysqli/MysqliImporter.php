 ...
 * @since  1.0
 */
class MysqliImporter extends AbstractDatabaseImporter
{
	/**
	...
	{
		$sql = 'ALTER TABLE ' . $this->database->quoteName($table) . ' ADD COLUMN ' . $this->getColumnSQL($field);
	...
	{
		$sql = 'ALTER TABLE ' . $this->database->quoteName($table) . ' ADD ' . $this->getKeySQL($keys);
	...
	{
		$sql = 'ALTER TABLE '
			. $this->database->quoteName($table)
			. ' CHANGE COLUMN '
			. $this->database->quoteName((string) $field->attributes()->Field)
	...
	protected function getColumnSQL(\SimpleXMLElement $field)
	{
		...
		$sql = $this->database->quoteName($fName) . ' ' . $fType;
		...
		if ($fNull == 'NO')
		{
			...
			else
			{
				...
				$sql .= ' NOT NULL DEFAULT ' . $this->database->quote($fDefault);
			}
		}
		else
		{
	...
	{
		$sql = 'ALTER TABLE ' . $this->database->quoteName($table) . ' DROP KEY ' . $this->database->quoteName($name);
	...
	{
		$sql = 'ALTER TABLE ' . $this->database->quoteName($table) . ' DROP PRIMARY KEY';
	...
	protected function getKeySQL($columns)
	{
		...
		if ($nColumns == 1)
		{
			$kColumns[] = $this->database->quoteName($kColumn);
		}
		else
		{
		...
		$sql = $prefix . 'KEY ' . ($kName != 'PRIMARY' ? $this->database->quoteName($kName) : '') . ' (' . implode(',', $kColumns) . ')';
	...
	protected function getInsertSQL(\SimpleXMLElement $structure)
	{
		...
		foreach ($structure->row->field as $field)
		{
			$cols[] = $field->attributes()->name;
		}
